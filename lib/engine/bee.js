...
class BeeSnapshot {
  ...
  getIndirectRange (reconstruct, entries, checkout, requests) {
    const promises = new Array(entries.length)

    for (let i = 0; i < promises.length; i++) {
      const { key, value } = entries[i]
      promises[i] = getWrapped(this.snapshot, key, reconstruct(key, value), checkout, requests)
    }

    return promises
  }

  getBatch (keys, checkout, requests) {
    const promises = new Array(keys.length)

    for (let i = 0; i < keys.length; i++) {
      promises[i] = getValue(this.snapshot, keys[i], checkout, requests)
    }

    return Promise.all(promises)
  }

  get (key, { checkout, activeRequests } = {}) {
    return getValue(this.snapshot, key, checkout, activeRequests)
  }
...
module.exports = class BeeEngine {
  ...
  clearRequests (reqs) {
    clearHypercoreRequests(reqs)
  }
}

async function getWrapped (db, key, value, checkout, requests) {
  return { key, value: [value, await getValue(db, value, checkout, requests)] }
}
...
function clearHypercoreRequests (requests) {
  while (requests.length > 0) {
    const ref = requests[requests.length - 1]
