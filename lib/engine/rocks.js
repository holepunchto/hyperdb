...
class RocksSnapshot {
  ...
  getIndirectRange(reconstruct, entries, checkout, reqs) {
    ...
    const read =
      this.batches.length > 0 ? this.batches.pop() : this.snapshot.read()
...
module.exports = class RocksEngine {
  constructor(storage, options = {}) {
    ...
    this.core = null
    this.db =
      typeof storage === 'object' ? storage : new RocksDB(storage, options)
  ...
  finalize(collection, version, checkout, tracing, key, value) {
    ...
    if (this.trace && tracing)
      this.trace(collection.name, reconstructed, checkout)
