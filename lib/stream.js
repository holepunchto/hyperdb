...
const Bee2Engine = require('./engine/bee2')
...
module.exports = class IndexStream extends Readable {
  constructor(
    ...
    this.snapshot = this._getSnapshot(db, checkout)
  ...
  _getSnapshot(db, checkout) {
    if (!needsCheckout(this.engine, checkout)) return db.engineSnapshot

    this.hasCheckout = true
    return db.engine.checkout(checkout)
  }
  ...
  _continueDestroyMaybe() {
    ...
    if (this.hasCheckout) this.snapshot.unref()
...
function needsCheckout(engine, checkout) {
  return engine instanceof Bee2Engine && !!checkout && checkout !== -1
}
